...
function ToastView({ toast }: { toast: Toast }) {
  ...
  return (
    <motion.div
      ...
      className="pointer-events-auto flex min-h-10 items-center gap-3 rounded-lg bg-[#1f1f1f] py-2 pl-3 pr-2 text-[14px] text-white/90 shadow-lg dark:bg-[#373737]"
    >
      ...
      <button onClick={() => dismiss(toast.id)} className="rounded p-1 text-white/40 hover:bg-white/10 hover:text-white/80">
        <X size={14} />
      </button>
    </motion.div>
  );
