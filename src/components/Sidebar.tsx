...
export function Sidebar() {
  ...
  return (
    <motion.aside
      ...
      className="relative z-20 h-full shrink-0 overflow-hidden border-r border-line bg-sidebar"
    >
      <div className="flex h-full flex-col" style={{ width }}>
        ...
        <div className="group/ws flex h-12 items-center gap-1 px-2 pt-1">
          ...
          <Tooltip content={t('nav.collapse')} shortcut={`${modKey()} \\`}>
            <IconButton
              className="opacity-0 transition-opacity group-hover/ws:opacity-100"
        ...
        <nav className="px-2 pb-2">
          <button
            ...
            className="flex h-[30px] w-full items-center gap-2 rounded-md px-2 text-[14px] text-fg-2 transition-colors hover:bg-hover"
          >
...
function Section({
  ...
  const addButton = (
    <IconButton size="xs" onClick={onAdd} label={addLabel} className="opacity-0 group-hover/sec:opacity-100 data-[state=open]:opacity-100">
      <Plus size={14} />
    </IconButton>
  );
  return (
    <div className="mt-3">
      <div className="group/sec flex h-[26px] items-center justify-between rounded-md px-2 transition-colors hover:bg-hover">
        <button onClick={() => setExpanded(key, collapsed)} className="flex flex-1 items-center gap-1 text-left text-[12px] font-medium text-fg-3">
          {title}
          <ChevronRight size={12} className={cn('opacity-0 transition-all group-hover/sec:opacity-100', !collapsed && 'rotate-90')} />
        </button>
...
function NavRow({
  ...
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        cn(
          'group/row relative flex h-[30px] items-center gap-2 rounded-md pr-1 text-[14px] transition-colors duration-100',
          isActive ? 'bg-active font-medium text-fg' : 'text-fg-2 hover:bg-hover',
        )
      }
...
function RowIcon({ icon, expandable, expanded, onToggle }: { icon: ReactNode; expandable?: boolean; expanded?: boolean; onToggle?: () => void }) {
  return (
    <span className="relative flex h-5 w-5 shrink-0 items-center justify-center text-fg-3">
      <span className={cn('flex items-center justify-center transition-opacity duration-100', expandable && 'group-hover/row:opacity-0')}>
        {icon}
      </span>
      {expandable && (
        <button
          ...
          className="absolute inset-0 flex items-center justify-center rounded opacity-0 transition-opacity duration-100 hover:bg-active group-hover/row:opacity-100"
        >
...
function RowActions({ children }: { children: ReactNode }) {
  return (
    <span
      className="flex items-center gap-0.5 opacity-0 transition-opacity duration-100 group-hover/row:opacity-100 has-[[data-state=open]]:opacity-100"
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
      }}
    ...
    <div className="mb-0.5">
      <ContextMenu entries={entries}>
        <div
          ...
          className={cn(
            'group/row relative flex h-[30px] cursor-pointer select-none items-center gap-2 rounded-md pl-2 pr-1 text-[14px] font-medium text-fg-2 transition-colors hover:bg-hover',
