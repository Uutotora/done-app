      ...
      {(collapsed || mobile) && (
        <Tooltip content={t('nav.expand')} shortcut={`${modKey()} \\`}>
          ...
          >
            ...
            <Menu size={18} className="absolute transition-[opacity,transform] duration-150 group-hover/menu:scale-75 group-hover/menu:opacity-0" />
            <ChevronsRight size={18} className="absolute -translate-x-1 opacity-0 transition-[opacity,transform] duration-150 group-hover/menu:translate-x-0 group-hover/menu:opacity-100" />
          </IconButton>
        </Tooltip>
      )}
